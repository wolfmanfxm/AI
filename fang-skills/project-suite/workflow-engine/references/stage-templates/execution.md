...
> Engine 拥有。Skill 通过 `@engine: execution` 引用，只需提供 Actions/Exit/Failure。
...
| Recovery | 读 manifest.json → 跳过 completed/failed subtasks → 只执行 pending/in_progress → [checkpoint protocol](../../../runtime/engine/checkpoint.md) |
...
- 详见 [checkpoint protocol](../../../runtime/engine/checkpoint.md)
