...
# 全部声明目录
KD_ALL="rules decisions patterns components api architecture experience playbooks recommendations observations conventions proposals reports candidates"

# 谁产出哪个目录（格式 dir:producer，多产出者逗号分隔）—— 与 artifact-types.yaml 的 producer 同义
KD_BY_PRODUCER="rules:human decisions:human,project-architect,project-analyzer patterns:project-analyzer components:project-analyzer api:project-analyzer architecture:project-analyzer experience:human playbooks:human recommendations:project-analyzer observations:project-analyzer conventions:project-analyzer proposals:project-planner reports:project-tester,project-reviewer,project-refactorer candidates:project-analyzer"
