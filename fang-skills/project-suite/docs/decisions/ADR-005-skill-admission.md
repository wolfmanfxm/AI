...
> ⚠️ **A5 管「能不能验证」，不管「值不值得存在」——两者是不同判据，勿混读。**
> A5 问的是「有没有可判定的 assertion」；而「去掉它仍得同等结果 → 不进核心」是 **ablation**，
> 判的是**机制**层面（Resolver / Registry / Gate / 新 prompt / 新 artifact / 新字段），
> 不是 skill 层面。后者**不在本 ADR**，见 [eval-contract.md 的「两种 baseline（ablation）」](../eval-contract.md)。
> 本节只把边界说清，不在此新增原则——ADR 是决策记录，清单式的准入条件会随时间腐坏。
