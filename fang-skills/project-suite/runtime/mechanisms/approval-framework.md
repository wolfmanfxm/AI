...
|-------|---------|--------|---------|
| **AUTO_PASS** | confidence ≥ `rules.yaml` 的 `gate.min_confidence_for_pass` | 无需人工确认，直接进入下一阶段 | ✅ 自动记录 |
| **CHECKPOINT** | Decision Boundary（用户决策会改变结果的地方：Goal/Scope 确认、技术选型、冲突裁决） | `AskUserQuestion` — 用户选"继续"/"调整"/"取消" | ✅ 用户选择写入 approval_log |
| **GATE** | confidence < `rules.yaml` 的 `gate.requires_review_below` | 强制 Review 后才能继续 | ✅ GATE 触发记录 |
| **BLOCK** | confidence < `rules.yaml` 的 `gate.blocks_downstream_below` | 拒绝下游执行 | ✅ BLOCK 原因记录 |
