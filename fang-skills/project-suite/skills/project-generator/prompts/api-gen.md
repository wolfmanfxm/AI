...
1. **查询 api 知识（query: api）** — 确认 request 封装路径和用法
2. **查询 api 知识（query: api）** — 确认请求格式（参数命名、响应结构）
3. **查询 api 模块知识（query: api）**（若存在）— 确认模块组织方式
...
- 导入项目的 request 封装（路径从 api/patterns 知识（query 注入）提取）
